#include "../inc/JoystickDataTransmitter.h"

#include <QLineEdit>

namespace FEITENG
{
    JoystickDataTransmitter::JoystickDataTransmitter(
        MainWindow* main_window_ptr) : QObject(nullptr)
    {
        m_main_window_ptr = main_window_ptr;

        m_host_ip = "192.168.16.102";
        m_main_window_ptr
            ->findChildControl<QLineEdit*>("host_ip_line_edit")
            ->setText(m_host_ip);
        connect(m_main_window_ptr
                    ->findChildControl<QLineEdit*>("host_ip_line_edit"),
                &QLineEdit::editingFinished,
                this,
                &JoystickDataTransmitter::changeHostIpByEdit);

        m_host_port = 10086;
        m_main_window_ptr
            ->findChildControl<QLineEdit*>("host_port_line_edit")
            ->setText(QString::number(m_host_port));
        connect(m_main_window_ptr
                    ->findChildControl<QLineEdit*>("host_port_line_edit"),
                &QLineEdit::editingFinished,
                this,
                &JoystickDataTransmitter::changeHostPortByEdit);

        m_socket_ptr = new QUdpSocket(this);

        qRegisterMetaType<QList<float>>();
    }

    JoystickDataTransmitter::~JoystickDataTransmitter()
    { }

    void JoystickDataTransmitter::transmitData(const QList<float>& robot_data)
    {
        auto crc8 = [](const QByteArray& data)
        {
            static const quint8 crc8_table[256] =
            {
                
            };
            quint8 crc = 0x00;
            for(const quint8 byte : data)
            {
                crc = crc8_table[crc ^ byte];
            }
            return crc;
        };

        QByteArray robot_data_byte_array;
        robot_data_byte_array
            .append(reinterpret_cast<const char*>(&robot_data.at(0)), 4)
            .append(reinterpret_cast<const char*>(&robot_data.at(1)), 4)
            .append(reinterpret_cast<const char*>(&robot_data.at(2)), 4)
            .append(reinterpret_cast<const char*>(&robot_data.at(3)), 4)
            .append(reinterpret_cast<const char*>(&robot_data.at(4)), 4)
            .append(reinterpret_cast<const char*>(&robot_data.at(5)), 4)
            .append(1)
            .append(reinterpret_cast<const char*>(&robot_data.at(6)), 4);
        quint8 crc = crc8(robot_data_byte_array);
        robot_data_byte_array.prepend(
            QByteArray().append(0xFA).append(0xAF).append(30)
        );
        robot_data_byte_array.prepend(
            QByteArray().append(crc).append(0xFB).append(0xBF)
        );
        m_socket_ptr->writeDatagram(robot_data_byte_array,
                                    QHostAddress(m_host_ip),
                                    m_host_port);
    }

    void JoystickDataTransmitter::changeHostIpByEdit()
    {
        m_host_ip = m_main_window_ptr
            ->findChildControl<QLineEdit*>("host_ip_line_edit")
            ->text();
    }

    void JoystickDataTransmitter::changeHostPortByEdit()
    {
        m_host_port = m_main_window_ptr
            ->findChildControl<QLineEdit*>("host_port_line_edit")
            ->text().toULongLong();
    }
} // namespace FEITENG
